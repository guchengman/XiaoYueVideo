<template>
  <div class="border border-gray-200 rounded-lg overflow-hidden">
    ...
    <div v-if="expanded" class="p-4 space-y-4">
      <!-- Platform selector -->
      <div class="flex items-center gap-2">
        <label class="text-sm text-gray-600 shrink-0">平台：</label>
      ...
      <!-- Export instructions -->
      <div v-if="showInstructions" class="bg-gray-50 rounded-lg p-3 sm:p-4 text-xs text-gray-600 space-y-3">
        ...
        <div>
          ...
          <code class="block bg-gray-800 text-green-400 p-2 rounded mt-1 whitespace-pre-wrap overflow-x-auto">python3 scripts\chrome_cookie_extract.py {{ selectedPlatform }} ..\cookies\{{ selectedPlatform }}_cookies.txt</code>
        </div>

        <div class="border-t border-gray-200 pt-2">
          <p class="font-medium text-gray-700 mb-1">✅ 导出成功后的效果（示例）：</p>
          <code class="block bg-gray-800 text-green-400 p-2 rounded text-[10px] leading-5 whitespace-pre-wrap overflow-x-auto"># Netscape HTTP Cookie File
